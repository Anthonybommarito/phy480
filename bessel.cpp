...
// When the error is near one it means the values of up and down are not close together. For x near 10 they are both accurate and the
//error is very small. There is a power law from x=1 to x=10. For x>50 the upward method is better.
#include <iostream>		// note that .h is omitted
#include <iomanip>		// note that .h is omitted
#include <fstream>		// note that .h is omitted
#include <cmath>
#include <gsl/gsl_sf_bessel.h> // include bessel module
  ...
  for (double x = xmin; x <= xmax; x += step)
    {
      ...
      error = abs(ans_up - ans_down)/(abs(ans_up)+abs(ans_down)); // error calculation between up/down
      double gsl=gsl_sf_bessel_J1(x); //bessel module computation

      my_out<< " "<<setprecision (12) << x;
      my_out << " " << setprecision (12) << ans_down;
      my_out << " "<< setprecision(12) << ans_up;
      my_out<<" "<<setprecision(12)<<error;
      my_out<<" "<<setprecision(12)<<gsl; //bessel module computation
      my_out<<endl;
    }
