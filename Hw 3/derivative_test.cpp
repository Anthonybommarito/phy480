//The error is roughly 1/N^6 like we expect when we look at values of x greater than -1. Ive attached two fits, one for the whole domain and one for x >= -1. plots were done in python, for some reason i got an error setting the terminal in gnu plot.
...
main (void)
{
  ...
  while (h >= hmin)
  {
    ...
    diff_extrap2 = extrap_diff2 (x, h, &test_function, params_ptr); //define extrap 2
    ...
    out << scientific << setprecision (8)
      ...
      << log10 (fabs ((diff_extrap2 - answer) / answer)) << endl; //extrap2 error
